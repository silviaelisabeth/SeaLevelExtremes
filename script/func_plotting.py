...
palette_func = plt.cm.RdBu_r 
CMAP = plt.cm.RdBu_r  # blue→white→red, white at 0
OUTLIER_COLOR = [200, 50, 200, 180] #< gold-yellow | [220, 50, 50, 180] #< red # [253, 253, 253, 180] #< white
OUTLIER_MARKER_SIZE = 6
FACTORMTOMM = 1000

#SIGNED_PARAMETERS = ["scale", "mu"]#["mu1", "shape"]
SIGNED_PARAMETERS = ["mu1", "shape", "loc_trend", "location_trend"]
POSITIVE_PARAMETERS = ["mu", "scale", "location", "sigma"]


CMAP_DIVERGING = cmc.vik          # or plt.cm.RdBu_r | blue→white→red, signed parameters: μ₁, ξ
CMAP_SEQUENTIAL = cmc.vik   # dark red to light orange, positive-only: σ, μ
CMAP = CMAP_DIVERGING             # backward-compat alias used in legend/norm functions


def is_signed_parameter(parameter):
    return parameter in SIGNED_PARAMETERS
...
def get_plot_cmap(parameter):
    if is_signed_parameter(parameter):
        return CMAP_DIVERGING   # blue→white→red
    return CMAP_SEQUENTIAL      # lajolla: pale yellow→deep red


def get_clean_parameter_data(df_plot, parameter, mark_outlier):
    if mark_outlier and "outliers" in df_plot.columns:
...
def get_norm_for_parameter(df_plot, parameter, mark_outlier, percentile=98):
    data = get_clean_parameter_data(df_plot, parameter, mark_outlier)

    lo = np.nanpercentile(data, 100 - percentile)
    hi = np.nanpercentile(data, percentile)
...
def convert_parameter_to_color(df_plot, mark_outlier, parameter):
    ...
    norm = get_norm_for_parameter(df, parameter, mark_outlier)
    cmap_plot = get_plot_cmap(parameter)

    def parameter_to_color(val):
        rgba = cmap_plot(norm(val))
        return [int(255 * c) for c in rgba[:3]]

    df["color"] = df[parameter].apply(parameter_to_color)

    return df


def convert_parameterSTD_to_alpha_v1(df_plot, parameter):
    """
    Higher uncertainty → higher transparency.
    """
    df = df_plot.copy()

    std_col = parameter + "_std"
    max_se = np.nanpercentile(np.abs(df[std_col]), 98)

    def se_to_alpha(se):
        if np.isnan(se) or np.isnan(max_se) or max_se == 0:
            return 255
    ...
    df["alpha"] = df[std_col].apply(se_to_alpha)

    return df

def convert_parameterSTD_to_alpha(df_plot, parameter, min_alpha=80, max_alpha=255):
    """
    ...
    def se_to_alpha(se):
        ...
        alpha = int(
            max_alpha - norm_se * (max_alpha - min_alpha)
        )
...
def create_gradient_legend(
    ...
    """
    HTML gradient legend matching the actual map colors.
    The input cmap argument is kept for backward compatibility but ignored.
    """
    cmap_plot = get_plot_cmap(parameter)
    parameter_pretty = get_parameter_pretty_name(parameter)
...
def get_clean_ticks(vmin, vmax, target_step=None):
    """
    Generate clean round-number ticks within [vmin, vmax],
    automatically choosing a step size based on the data range.
    """
    data_range = vmax - vmin

    if target_step is None:
        # Pick a clean step: 0.1, 0.25, 0.5, 1, 2, 5 ...
        raw_step = data_range / 6
        magnitude = 10 ** np.floor(np.log10(raw_step))
        for nice in [1, 2, 2.5, 5, 10]:
            step = nice * magnitude
            if data_range / step <= 8:
                break
    else:
        step = target_step
    ...
    # Always include 0 for diverging colorbars
...
def add_parameter_colorbar(fig, ax, parameter, norm, unit):
    cmap_plot = get_plot_cmap(parameter)
    sm = ScalarMappable(norm=norm, cmap=cmap_plot)
    sm.set_array([])

    cbar = fig.colorbar(sm, ax=ax, fraction=0.035, pad=0.02, extend='neither')

    ticks = get_clean_ticks(norm.vmin, norm.vmax)
    cbar.set_ticks(ticks)
    cbar.ax.tick_params(labelsize=8)

    parameter_pretty = get_parameter_pretty_name(parameter)
    cbar.set_label(f"{parameter_pretty}, {unit}")
    return cbar
...
def get_plot_cmap_v0(parameter):
    """
    Signed parameters:
        full vik (blue-white-red)

    Positive-only parameters:
        only white->red half of vik
    """
    if is_signed_parameter(parameter):
        return CMAP
    ...
    return mcolors.LinearSegmentedColormap.from_list(
        "vik_positive",
        CMAP(np.linspace(0.5, 1.0, 256))
    )


def get_norm_for_parameter_v0(df_plot, parameter, mark_outlier, percentile=98):
    if mark_outlier:
        data = df_plot.loc[~df_plot["outliers"], parameter]
    else:
        data = df_plot[parameter]
...
def get_norm_for_parameter_v1(df_plot, parameter, mark_outlier):
    if mark_outlier:
        data = df_plot.loc[~df_plot["outliers"], parameter]
    else:
        data = df_plot[parameter]
...
'''
def equivalent_return_period_ns(params, years_mean, z_ref, year_future):
    mu0, mu1, sigma, xi = params
    
    t = year_future - years_mean
    mu_t = mu0 + mu1 * t
...
def delta_method_return_period(ns_model, z_ref, year_future):
    ...
    def g(p):
        return equivalent_return_period_ns(
            p, years_mean, z_ref, year_future
        )
...
def convert_parameter_to_color_v0(df_plot, mark_outlier, parameter):

    df = df_plot.copy()

    norm = get_norm_for_parameter(df_plot, parameter, mark_outlier)
    cmap = get_plot_cmap(parameter)
...
def convert_parameterSTD_to_alpha_v0(df_plot, parameter):
    """
...
def custom_color_for_parameter_v0(df_plot, mark_outlier, parameter, min_size=10, max_size=200):
    df_plot = convert_parameter_to_color(df_plot, mark_outlier, parameter)
    df_plot = convert_parameterSTD_to_alpha(df_plot, parameter)
...
def create_gradient_legend_v0(
    ...
    if mark_outlier is True:
        ...
        ">
            ...
            <div style="display:flex;justify-content:space-between;">
                <span>{norm.vmin:.2f}</span>
                middle_label = "0" if is_signed_parameter(parameter) else ""
                <span>{norm.vmax:.2f}</span>
            </div>
            <br>
            <b>Transparency</b><br>
...
def initialize_projection_v0(df_clean, parameter, min_size, max_size, mark_outlier):
    gdf = gpd.GeoDataFrame(
...
def plot_map_of_parameter_for_all_sites_static(
    ...
    ax.set_axis_off()
    cmap_plot = get_plot_cmap(parameter)
    sm = ScalarMappable(norm=norm, cmap=get_plot_cmap(parameter))
    sm.set_array([])
...
def plot_return_period_evolution_1961_50yr(
    rp_mc,
    ...
    ax.plot(
        ...
        lw=2,
        label="Equivalent return period, median",
    )
